...
with open('fig6b.base.%s.csv' % (in_fname), 'w') as outf:
	pass
with open('fig6c.base.%s.csv' % (in_fname), 'w') as outf:
	pass
for num_static in STATIC_VALUES:
	with open('fig6b.dyn.S%d.%s.csv' % (num_static, in_fname), 'w') as outf:
		pass
	with open('fig6b.pfl.S%d.%s.csv' % (num_static, in_fname), 'w') as outf:
		pass
	with open('fig6c.dyn.S%d.%s.csv' % (num_static, in_fname), 'w') as outf:
		pass
	with open('fig6c.pfl.S%d.%s.csv' % (num_static, in_fname), 'w') as outf:
		pass
...
with open(sys.argv[1]) as f:
	...
	with open('fig6b.base.%s.csv' % (in_fname), 'a') as outf:
		outf.write( '%f %d\n' % (0.0, 0) )
	with open('fig6c.base.%s.csv' % (in_fname), 'a') as outf:
		outf.write( '%f %f\n' % (0.0, 0.0) )
	for num_static in STATIC_VALUES:
		num_extra_flows = len([x for x,v in flow_counts.items() if v > num_static])
		with open('fig6b.pfl.S%d.%s.csv' % (num_static, in_fname), 'a') as outf:
			outf.write( '%f %d\n' % (0.0, 0) )
		with open('fig6c.dyn.S%d.%s.csv' % (num_static, in_fname), 'a') as outf:
			outf.write( '%f %f\n' % (0.0, 0.0) )
		with open('fig6c.dyn.S%d.%s.csv' % (num_static, in_fname), 'a') as outf:
			outf.write( '%f %f\n' % (0.0, 0.0) )
		with open('fig6c.pfl.S%d.%s.csv' % (num_static, in_fname), 'a') as outf:
			outf.write( '%f %f\n' % (0.0, 0.0) )
	...
	for line in f:
		...
		if cur_ts - begin_time > GRAPH_RESOLUTION:
			...
			for num_static in STATIC_VALUES:
				...
				with open('fig6b.dyn.S%d.%s.csv' % (num_static, in_fname), 'a') as outf:
