...
if len(sys.argv) < 2:
    print("Usage: find_last_flow_times trace.txt")

infname = sys.argv[1].split('/')[-1][:-4]

my_print("Working with %s" % infname)

i = 0
flows = {}

my_print("Reading %s..." % infname)
...
my_print('Writing to traces/%s.lasttimes.txt' % infname)
with open('traces/%s.lasttimes.txt' % infname, 'w') as outf:
